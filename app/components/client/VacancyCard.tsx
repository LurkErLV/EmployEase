    ...
    <>
      <Link
        ...
        <div className="flex gap-2 mb-5 max-sm:flex-col">
          <p className="text-[#0BA02C] text-xs font-bold uppercase bg-[#E7F6EA] px-[8px] py-[4px] w-fit">
            {formatWorkSchedule(props.item.workSchedule)}
          </p>
          <p className="text-gray-500 text-sm">
            Salary: {formatMoney(props.item.minSalary)} -{' '}
            {formatMoney(props.item.maxSalary)}
          </p>
        </div>
        <div className="flex gap-3">
          <div className="w-12 h-12 rounded bg-[#EDEFF5] flex justify-center items-center">
            <Image
              src={
                props.item.companyLogo.startsWith('http') ||
                props.item.companyLogo.startsWith('https')
